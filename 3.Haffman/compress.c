...
HufNode **huf_tree;
unsigned char code_buf[MAX_SIZE] = "\0";		// 待存编码缓冲区
...
int black_file(const char *outFileName) {
	...
	fwrite((char *)&linefeed_num, sizeof(unsigned int), 1, outfile);		// 写入换行符个数
...
void out_file_info(const char *outFileName) {
    ...
    FILE *f = fopen("3_3.txt", "wb");
	fwrite((char *)&linefeed_num, sizeof(unsigned int), 1, outfile);		// 写入换行符个数
	fwrite((char *)&char_kinds, sizeof(unsigned int), 1, outfile);		// 写入字符种类
	for(i = 0; i < char_kinds; ++i) {
		fwrite((char *)&tmp_nodes[i].uch, sizeof(unsigned char), 1, outfile);			// 写入字符（已排序，读出后顺序不变）
		fwrite((char *)&tmp_nodes[i].weight, sizeof(unsigned long), 1, outfile);		// 写入字符对应权重
        printf("ch: %c, ww: %ld\n", tmp_nodes[i].uch, tmp_nodes[i].weight);
        fwrite((char *)&tmp_nodes[i].uch, sizeof(unsigned char), 1, f);
	}
    fclose(f);
...
int compress(const char *inFileName, const char *outFileName) {
    ...
    DBG(YELLOW"<char kinds>"NONE" : get char kinds is success, kinds is %d, line is %d...\n", char_kinds, linefeed_num);

    if ((char_kinds == 1 && linefeed_num == 1) || char_kinds == 0 && linefeed_num == 0){ //空文件
        DBG(BLUE"BLACK\n"NONE);
        if (black_file(outFileName) == -1) return -1;
    } else if ((char_kinds == 2 && linefeed_num == 1) || (char_kinds == 1 && linefeed_num == 0)) { //单个字符
        DBG(BLUE"single"NONE);    
