/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2021年01月30日 星期六 22时58分02秒
 ************************************************************************/
	...
	*/
	...
	else
	{
        ...
        for(i = 0; i < char_kinds; ++i)
		{
			...
			fwrite((char *)&huf_tree[i].uch, sizeof(unsigned char), 1, f);			// 写入字符（已排序，读出后顺序不变）
		}
        fclose(f);
...
// 解压函数
int decompression(char *ifname, char *ofname)
{
	unsigned int i;
	...
	infile = fopen(ifname, "r");		// 以二进制方式打开压缩文件
	// 判断输入文件是否存在
	if (infile == NULL)
		return -1;
	...
	if (char_kinds == 1)
	{
		...
		outfile = fopen(ofname, "w");					// 打开压缩后将生成的文件
		while (file_len--)
